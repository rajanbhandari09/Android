 ...
 */
public class ActivityPlacesOfInterestIndianaPolis extends AppCompatActivity implements FragmentPlacesOfInterest.ListSelectionListener{
    //Array of strings to hold places of interest in IndianaPolis
    public static String[] placesofinterest;
    //Array of strings to hold websites for places of interest in IndianaPolis
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        placesofinterest = getResources().getStringArray(R.array.indianapolisplacesofinterest);
        placesofinterestwebsites = getResources().getStringArray(R.array.indianapolisplacesofinterestwebsites);
...
// This may happen because activity was restarted due device configuration change but the fragments were still retained due to setRetainInstance set to true
        mfragmentPlacesOfInterest = (FragmentPlacesOfInterest)mFragmentManager.findFragmentByTag("placesofinterestIndianaPolis");
        mfragmentwebpage = (FragmentWebPage)mFragmentManager.findFragmentByTag("webpageIndianaPolis");
//Actions when current device configuration is set to Portrait
        if(getResources().getConfiguration().orientation== Configuration.ORIENTATION_PORTRAIT){
            ...
            if (mfragmentPlacesOfInterest==null&&mfragmentwebpage==null){
                ...
                transaction.add(R.id.placesofinterest, mfragmentPlacesOfInterest, "placesofinterestIndianaPolis");
                transaction.commit();
...
//defining backstackchange listener when device is in landscape mode
    ...
    public void onListSelection(int index) {
        if(!mfragmentwebpage.isAdded()){
            FragmentTransaction transaction = mFragmentManager.beginTransaction();
            transaction.add(R.id.website, mfragmentwebpage,"webpageIndianaPolis");
    ...
    public boolean onOptionsItemSelected(MenuItem item) {
        ...
        if (id == R.id.action_chicago) {
            ...
            Intent intent = new Intent(this,ActivityPlacesOfInterestChicago.class);
